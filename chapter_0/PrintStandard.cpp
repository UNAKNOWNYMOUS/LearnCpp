...
long getCPPStandard() {
  // Visutal Studio is non-conforming in support for __cplusplus (unless you set
  // a specfic compiler flag, which you probably haven't) In Visual Studio 2015
  // or newer we can use _MSVC_LANG instead See
  // https://devblogs.microsoft.com/cppblog/mscv-now-correctly-reports-__cplusplus/
#if defined(_MSVC_LANG)
  return _MSVC_LANG;
#elif defined(_MSVC_VER)
  // If we're using an older verion of Visual Studio, bail out
  return -1;
#else
  // __cplusplus is the intended way to query the language standard code (as
  // defined by the languade standards)
  return __cplusplus;
#endif
}
...
int main() {
  ...
  for (int i = 0; i < numStandards; ++i) {
    ...
    // If the reported version is between two finalized standard code,
